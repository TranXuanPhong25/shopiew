...
export const NewProductFormSchema = z.object({
	...
	category: z
		.string()
		.nonempty({ message: "Product's category is required." }),
	description: z
		.string()
		.min(1, { message: "Product's description is required." }),
	specs: z.object({
		brand: z.string().min(1, { message: "Thương hiệu không được để trống." }),
		packageSize: z.string().refine((val) => !val || /^\d+$/.test(val), {
			message: "Package size must be a integer.",
		}),
		activeIngredients: z.string().optional(),
		ingredients: z.string().optional(),
		quantity: z.string().refine((val) => !val || /^\d+$/.test(val), {
	...
	salePrice: z.string().refine((val) => !val || /^\d+(\.\d+)?$/.test(val), {
		message: "Giá bán phải là số.",
	}),
	stockQuantity: z.string().refine((val) => !val || /^\d+$/.test(val), {
		message: "Kho hàng phải là số.",
	}),
	...
	shippingInfo: z
		.object({
			weightAfterPackaging: z
				.string()
				.refine((val) => !val || /^\d+(\.\d+)?$/.test(val), {
					message: "Trọng lượng sau khi đóng gói phải là số.",
				})
				.optional(),
			dimensions: z
				.object({
