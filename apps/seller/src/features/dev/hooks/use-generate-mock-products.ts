...
export function useGenerateMockProducts() {
	...
	const generateMockProducts = async (
	...
	): Promise<GenerateMockProductsResult> => {
		...
		if (!shopId) {
			...
			return {
				...
				count: 0,
				products: [],
				errors: [{ index: -1, error: "Shop ID is required" }],
			};
		}
			...
			for (let i = 0; i < count; i++) {
				try {
					...
					const createProductData: CreateProductData = {
						...
						variants: [
							{
								originalPrice: mockProductData.variant.price + "",
								salePrice: mockProductData.variant.price + "",
								stockQuantity:
									mockProductData.variant.stockQuantity + "",
					...
					const createdProduct =
						await ProductsService.createDraftProduct(createProductData);
					...
					toast.success(
						`Created product ${i + 1}/${count}: ${mockProductData.product.name}`,
					);
				} catch (err) {
...
function generateMockProduct(
	...
	const randomIndex = Math.floor(
		Math.random() *
			Math.min(productNames.length - 1, descriptions.length - 1),
	);
	const name =
		productNames[randomIndex] + " " + Math.floor(Math.random() * 10000);
	...
	const product = {
		...
		coverImage: includeImages
			? `https://picsum.photos/seed/${uuid}/640/480`
			: "",
		images: includeImages
			? Array.from(
					{ length: Math.floor(Math.random() * 5) + 1 },
					(_, i) => `https://picsum.photos/seed/${uuid}-${i}/640/480`,
				)
			: [],
		description,
