		...
		(acc, option) => {
			if (acc.length === 0) {
				return option.values
					.filter((valueOption) => valueOption.value.trim() !== "")
					.map((value) => [{ name: option.name, value: value.value }]);
			}
	...
	devtools(
		(set, get) => ({
				...
				const mergedVariants = newVariants.map((newVariant) => {
					...
					if (existingVariant) {
						...
						return {
							...
							originalPrice: existingVariant.originalPrice, // Preserve original price
							salePrice: existingVariant.salePrice, // Preserve sale price
			...
			deleteOption: (optionId) => {
				const newOptions = get().options.filter(
			...
			updateValue: (optionId, valueId, value) => {
				...
				const isLastValueEmpty = get().options.find(
					(option) =>
						option.id === optionId &&
			...
			hasSelectedVariants: () =>
				get().variants.some((variant) => variant.selected),
			variantsSelectState: () => {
				const selectedCount = get().variants.filter(
			...
			onImageChange: (variantId, file) => {
				...
				const newVariants = get().variants.map((variant) =>
					variant.id === variantId
						? {
								...variant,
								images: [...(variant.images || []), ...newVariantImage],
							}
						: variant,
				);
				set({ variants: newVariants });
			},
			getSelectedVariantsHasImage: () =>
				get().variants.filter(
					(variant) =>
						variant.selected &&
						variant.images &&
